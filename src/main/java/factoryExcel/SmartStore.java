...
public class SmartStore {
    ...
    public SmartStore(String brandNameKor, String categoryId, List<BaseItemSimpleCosmetic> baseItemSimpleCosmetic) {
    ...
    public void createExcelCosmetic(String dirExcelFile) {
        LOGGER.info("Creating the excel for smartstore starts...");
        HSSFWorkbook workbook = new HSSFWorkbook();
        HSSFSheet sheet = workbook.createSheet("cosmetic");
        ...
        for (int i=0;i<baseItemSimpleCosmetic.size();i++) {
            Object createdItemRow[] = createItemRowCosmetic(baseItemSimpleCosmetic.get(i));
            data.put(String.valueOf(i+1), createdItemRow);
        }
        ...
        try {
            FileOutputStream outputStream = new FileOutputStream(dirExcelFile + brandNameKor + "_smartstore.xlsx");
            workbook.write(outputStream);
            workbook.close();
        } catch (FileNotFoundException e) {
            e.printStackTrace();
...
//        HSSFSheet sheet = workbook.createSheet(itemFeelway.getItemTitleEng());
    ...
    private Object[] createItemRowCosmetic(BaseItemSimpleCosmetic item) {
        Object itemRow[] = new Object[66];
        //상품상태
        itemRow[0] = "신상품";
        //카태고리 id
        ...
        //판매가
        itemRow[3] = String.valueOf(item.getMassItem().getItemPriceWon());
        //재고수량
        itemRow[4] = 10;
        //A/S안내내용
        itemRow[5] = "구매대행 특성상 AS는 불가합니다";
        ...
        itemRow[6] = "070-4001-8993";
        //대표 이미지 파일명
        itemRow[7] = item.getMassItem().getMainImageFileName();
        //추가 이미지 파일명
        itemRow[8] = "";
